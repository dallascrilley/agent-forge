...
| `llm_optional` | bool | `true` | A run with nothing to do must complete without invoking the model. pi-mono cron sitters enforce this in `run.sh` (empty gather → quiet receipt, no `pi`). |
